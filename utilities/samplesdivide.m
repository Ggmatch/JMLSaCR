function [DataTest DataTrain CTest CTrain Loc_test] = samplesdivide(indian_pines_corrected,indian_pines_gt,train_num, train_ratio,randpp,database);
...
for i = 1:max(indian_pines_gt(:))
    ...
    if strcmp(database,'Indian')
        cTrain  = round(train_ratio*ci);  %Indian
        if cTrain > train_num
            cTrain = train_num;
        end
    else
